...
class MaturityRubric:
    ...
    # Variance thresholds
    VARIANCE_THRESHOLDS = {
        'low': (0, 0.6, 'Green', 'Strong alignment'),
        'medium': (0.6, 1.2, 'Yellow', 'Some disagreement on effectiveness'),
        'high': (1.2, float('inf'), 'Red', 'Significant perception gaps requiring intervention')
    }
    ...
    def calculate_dimension_score(self, responses: Dict[str, float],
                                  role: str, dimension: str) -> float:
        """
    ...
    def calculate_weighted_dimension_score(self,
                                          ceo_score: float,
                                          tech_score: float,
                                          staff_score: float,
                                          dimension: str) -> float:
        ...
        """
        weights = self.STAKEHOLDER_WEIGHTS.get(dimension, {'CEO': 0.33, 'Tech Lead': 0.33, 'Staff': 0.34})
    ...
    def generate_recommendations(self, overall_score: float,
                                ...
                                variance_analysis: Dict[str, Dict]) -> List[str]:
        """
        ...
        if 'Building' in maturity_level:
            ...
            recommendations.append("Action: Establish basic infrastructure foundation (reliable internet, cloud email)")
            recommendations.append("Support: Engage in fundamental training program for staff")
            ...
            recommendations.append("Support: Advance staff training to intermediate technology skills")
            recommendations.append("Timeline: 6-12 month targeted support program recommended")

        else:  # Thriving
            recommendations.append("Opportunity: Participate in innovation lab and pilot advanced technologies")
            recommendations.append("Leadership: Consider peer mentorship role for other organizations")
            recommendations.append("Growth: Explore AI/ML applications for mission enhancement")
            recommendations.append("Recognition: Strong candidate for sector leadership opportunities")
        ...
        if valid_dimension_scores:
            ...
            if dimension_score < 2.5:
                if dimension_name == 'Program Technology':
                    recommendations.append(f"Critical Gap ({dimension_name}): Invest in program delivery technology to support mission")
                elif dimension_name == 'Business Systems':
                    recommendations.append(f"Critical Gap ({dimension_name}): Integrate core business systems (CRM, finance, HR)")
                elif dimension_name == 'Data Management':
                    recommendations.append(f"Critical Gap ({dimension_name}): Establish data collection and analysis capabilities")
                elif dimension_name == 'Infrastructure':
                    recommendations.append(f"Critical Gap ({dimension_name}): Upgrade network and cybersecurity infrastructure")
                elif dimension_name == 'Organizational Culture':
                    recommendations.append(f"Critical Gap ({dimension_name}): Build technology-positive culture through leadership engagement")

        # High variance recommendations
        high_variance_dims = [dim for dim, analysis in variance_analysis.items()
                             if analysis.get('level') == 'high']

        if high_variance_dims:
            dim_list = ', '.join(high_variance_dims)
            recommendations.append(f"Alignment Needed: Significant perception gaps in {dim_list} - recommend stakeholder alignment sessions")
    ...
    def calculate_overall_maturity(self, org_responses: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Calculate overall score (only from dimensions with valid scores)
        valid_dimensions = [(d, score) for d, score in dimension_scores.items() if score is not None]

        if valid_dimensions:
            total_weighted_score = sum(score * self.DIMENSION_WEIGHTS[d] for d, score in valid_dimensions)
            total_weight = sum(self.DIMENSION_WEIGHTS[d] for d, _ in valid_dimensions)
