...
from ..repositories.spreadsheet_repository import (
    SpreadsheetRepository, SheetRepository, SheetColumnRepository, 
    SheetRowRepository, SheetCellRepository, DataExtractionJobRepository
)
from ..models.models import (
    Spreadsheet, Sheet, SheetColumn, SheetRow, SheetCell, DataExtractionJob
)
...
class IDataExtractionService(ABC):
    ...
    @abstractmethod
    def extract_and_normalize(self, urls: List[str], job_name: Optional[str] = None) -> DataExtractionJob:
...
class DataExtractionService(IDataExtractionService):
    ...
    def _process_sheet(self, spreadsheet: Spreadsheet, sheet_data: SheetData) -> int:
        ...
        sheet = self.sheet_repo.get_by_spreadsheet_and_name(
            spreadsheet.id, sheet_data.sheet_name
        )
    ...
    def _process_rows_and_cells(self, sheet: Sheet, columns: List[SheetColumn], rows_data: List[List[Any]]) -> int:
