...
class ReportGenerator:
    ...
    def generate_organization_report(self, org_name: str) -> Optional[Dict[str, Any]]:
        ...
        report = {
            ...
            'timeline': self._build_org_timeline(intake_record, ceo_record, tech_records, staff_records),
    ...
    def generate_aggregate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'overview': self._build_aggregate_overview(intake_data, ceo_data, tech_data, staff_data),
            'breakdown': self._build_aggregate_breakdown(ceo_data, tech_data, staff_data),
            'timeline': self._build_aggregate_timeline(intake_data, ceo_data, tech_data, staff_data),
            ...
            'recommendations': self._build_aggregate_recommendations(intake_data, ceo_data, tech_data, staff_data)
        }
    ...
    def _build_org_header(self, org_name: str, intake_record: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'intake_date': intake_record.get('Date', 'N/A')[:10] if intake_record.get('Date') else 'N/A',
            'generated_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }

    def _build_org_summary(self, ceo_record: Optional[Dict],
                          tech_records: List[Dict],
                          staff_records: List[Dict]) -> Dict[str, Any]:
    ...
    def _build_org_timeline(self, intake_record: Dict,
                           ceo_record: Optional[Dict],
                           tech_records: List[Dict],
                           staff_records: List[Dict]) -> List[Dict[str, Any]]:
    ...
    def _build_org_contacts(self, ceo_record: Optional[Dict],
                           tech_records: List[Dict],
                           staff_records: List[Dict]) -> List[Dict[str, Any]]:
        ...
        if ceo_record:
            contacts.append({
                ...
                'submission_date': ceo_record.get('Date', '')[:10] if ceo_record.get('Date') else None
            })
    ...
    def _build_org_intake_insights(self, intake_record: Dict) -> Dict[str, Any]:
        ...
        return {
            'ai_usage': intake_record.get('Please select which of these best describes how AI is currently being used in your organization:', 'Not specified'),
            'ai_policy': intake_record.get('Do you have an AI policy in place?', 'Not specified'),
            'comments': intake_record.get('Do you have any suggestions or comments for us on the Technology Strategy?', ''),
            'raw_data': intake_record
        }

    def _build_org_responses(self, ceo_record: Optional[Dict],
                            tech_records: List[Dict],
                            staff_records: List[Dict]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _build_aggregate_overview(self, intake_data: List[Dict],
                                  ceo_data: List[Dict],
                                  tech_data: List[Dict],
                                  staff_data: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'completion_percentage': round(((ceo_complete + tech_complete + staff_complete) / (total_orgs * 3)) * 100) if total_orgs > 0 else 0,
    ...
    def _build_aggregate_breakdown(self, ceo_data: List[Dict],
                                   tech_data: List[Dict],
                                   staff_data: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            'by_survey_type': {
                'CEO': {
                    ...
                    'total_responses': sum(len([k for k in r.keys() if k.startswith('C-')]) for r in ceo_data if r.get('Date'))
                },
                'Tech Lead': {
                    'completed': len([r for r in tech_data if r.get('Date')]),
                    'pending': len([r for r in tech_data if not r.get('Date')]),
                    'total_responses': sum(len([k for k in r.keys() if k.startswith('TL-')]) for r in tech_data if r.get('Date'))
                },
                'Staff': {
                    'completed': len([r for r in staff_data if r.get('Date')]),
                    'pending': len([r for r in staff_data if not r.get('Date')]),
                    'total_responses': sum(len([k for k in r.keys() if k.startswith('S-')]) for r in staff_data if r.get('Date'))
                }
            }
        }

    def _build_aggregate_timeline(self, intake_data: List[Dict],
                                  ceo_data: List[Dict],
                                  tech_data: List[Dict],
                                  staff_data: List[Dict]) -> List[Dict[str, Any]]:
    ...
    def _build_aggregate_table(self, intake_data: List[Dict],
                               ceo_data: List[Dict],
                               tech_data: List[Dict],
                               staff_data: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for intake in intake_data:
            ...
            org_status.append({
                ...
                'completion_percentage': completion_pct,
                'overall_status': 'Complete' if completed == 3 else ('In Progress' if completed > 0 else 'Not Started')
            })
    ...
    def _build_aggregate_insights(self, ceo_data: List[Dict],
                                  tech_data: List[Dict],
                                  staff_data: List[Dict]) -> Dict[str, Any]:
    ...
    def _build_aggregate_recommendations(self, intake_data: List[Dict],
                                        ceo_data: List[Dict],
                                        tech_data: List[Dict],
                                        staff_data: List[Dict]) -> List[str]:
        ...
        if ceo_complete < total_orgs * 0.5:
            recommendations.append(f"Focus on CEO survey completion: Only {ceo_complete}/{total_orgs} ({round((ceo_complete/total_orgs)*100)}%) completed")

        # Tech completion rate
        if tech_complete < total_orgs * 0.5:
            recommendations.append(f"Increase Tech Lead engagement: Only {tech_complete} surveys completed")

        # Staff completion rate
        if staff_complete < total_orgs * 0.5:
            recommendations.append(f"Prioritize Staff survey collection: Only {staff_complete} responses received")

        # Overall completion
        total_expected = total_orgs * 3
        total_complete = ceo_complete + tech_complete + staff_complete
        if total_complete < total_expected * 0.75:
            recommendations.append(f"Overall survey completion is {round((total_complete/total_expected)*100)}% - target 75%+ for robust analysis")

        if not recommendations:
            recommendations.append("Strong survey completion rates across all categories - continue current outreach efforts")
